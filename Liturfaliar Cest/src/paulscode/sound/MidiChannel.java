...
/**
 * The MidiChannel class provides an interface for playing MIDI files, using the JavaSound API. For more information about the JavaSound API, visit http://java.sun.com/products/java-media/sound/ <br>
 * <br>
...
public class MidiChannel implements MetaEventListener {
  ...
  public void cleanup() {
    ...
    synchronized (sequenceQueueLock) {
      if (sequenceQueue != null) sequenceQueue.clear();
      sequenceQueue = null;
    }
  ...
  public void queueSound(FilenameURL filenameURL) {
    ...
    synchronized (sequenceQueueLock) {
      if (sequenceQueue == null) sequenceQueue = new LinkedList<FilenameURL>();
  ...
  private synchronized FilenameURL filenameURL(boolean action, FilenameURL value) {
    if (action == SET) filenameURL = value;
    return filenameURL;
  }
  ...
  private void getSequencer() {
    ...
    if (sequencer == null) sequencer = openSequencer("Real Time Sequencer");
    if (sequencer == null) sequencer = openSequencer("Java Sound Sequencer");
    if (sequencer == null) {
  ...
  private void getSynthesizer() {
    ...
    if (sequencer instanceof Synthesizer) {
      ...
      if (synthesizer == null) {
        ...
        if (synthDevice == null) synthDevice = openMidiDevice("Microsoft GS Wavetable");
        if (synthDevice == null) synthDevice = openMidiDevice("Gervill");
        if (synthDevice == null) {
  ...
  private MidiDevice openMidiDevice(String containsString) {
    ...
    for (int i = 0; i < midiDevices.length; i++) {
      ...
      if (device != null && midiDevices[i].getName().contains(containsString)) {
        ...
        if (device instanceof Synthesizer) message("        *this is a Synthesizer instance");
        if (device instanceof Sequencer) message("        *this is a Sequencer instance");
        try {
          device.open();
        }
        catch (MidiUnavailableException mue) {
  ...
  private class FadeThread extends SimpleThread {
    ...
    public void run() {
      while (!dying()) {
        ...
        if (fadeOutGain == -1.0f && fadeInGain == 1.0f) snooze(3600000);
        checkFadeOut();
