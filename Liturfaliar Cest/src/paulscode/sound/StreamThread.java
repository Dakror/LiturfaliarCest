...
/**
 * The StreamThread class is used to process all streaming sources. This thread starts out asleep, and it sleeps when all streaming sources are finished playing, so it is necessary to call interrupt() after adding new streaming sources to the list. <br>
 * <br>
        ...
        synchronized (listLock) {
          ...
          while (!dying() && iter.hasNext()) {
            ...
            if (src == null) {
              ...
              if (!src.rawDataStream) iter.remove();
            } else if (!src.active()) {
              ...
              if ((!src.stream()) && (!src.rawDataStream)) {
                if (src.channel == null || !src.channel.processBuffer()) {
                  ...
                  /*
                   * if( src.getSoundSequenceQueueSize() > 0 ) { src.incrementSoundSequence(); } // check if this is a looping source else
                   */if (src.toLoop) {
