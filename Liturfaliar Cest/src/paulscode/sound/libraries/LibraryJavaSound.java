...
/**
 * The LibraryJavaSound class interfaces the JavaSound library. For more information about the JavaSound API, visit http://java.sun.com/products/java-media/sound/ <br>
 * <br>
 * <b><i> SoundSystem LibraryJavaSound License:</b></i><br>
 * <b><br>
 * You are free to use this library for any purpose, commercial or otherwise. You may modify this library or source code, and distribute it any way you like, provided the following conditions are met: <br>
 * 1) You may not falsely claim to be the author of this library or any unmodified portion of it. <br>
 * 2) You may not copyright this library or a modified version of it and then sue me for copyright infringement. <br>
 * 3) If you modify the source code, you must clearly document the changes made before redistributing the modified source code, so other users know it is not the original code. <br>
 * 4) You are not required to give me credit for this library in any derived work, but if you do, you must also mention my website: http://www.paulscode.com <br>
 * 5) I the author will not be responsible for any damages (physical, financial, or otherwise) caused by the use if this library or any part of it. <br>
 * 6) I the author do not guarantee, warrant, or make any representations, either expressed or implied, regarding the use of this library or any part of it. <br>
 * <br>
 * Author: Paul Lamb <br>
...
public class LibraryJavaSound extends Library {
    ...
    if (myMixer == null) {
      ...
      if (myMixer == null) {
        ...
        for (Mixer.Info mixerInfo : AudioSystem.getMixerInfo()) {
          ...
          if (bestRankedMixer == null || mixerRanker.rank > bestRankedMixer.rank) bestRankedMixer = mixerRanker;
        }
        // Check if didn't find any useable mixers at all:
        if (bestRankedMixer == null) throw new LibraryJavaSound.Exception("No useable mixers " + "found!", new MixerRanking());
        try {
  ...
  public static class MixerRanking {
    ...
    public void rank(Mixer.Info i) throws LibraryJavaSound.Exception {
      ...
      if (i == null) throw new LibraryJavaSound.Exception("No Mixer info " + "specified in method 'MixerRanking.rank'", this);
      mixerInfo = i;
      Mixer m;
      try {
      ...
      if (m == null) throw new LibraryJavaSound.Exception("Unable to acquire the " + "specified Mixer in method 'MixerRanking.rank'", this);
      mixerExists = true;
      ...
      if (!AudioSystem.isLineSupported(lineInfo)) {
        if (MIN_SAMPLE_RATE_PRIORITY == HIGH) throw new LibraryJavaSound.Exception("Specified minimum " + "sample-rate not possible for Mixer '" + mixerInfo.getName() + "'", this);
      } else {
      ...
      if (!AudioSystem.isLineSupported(lineInfo)) {
        if (MAX_SAMPLE_RATE_PRIORITY == HIGH) throw new LibraryJavaSound.Exception("Specified maximum " + "sample-rate not possible for Mixer '" + mixerInfo.getName() + "'", this);
      } else {
      ...
      if (minSampleRatePossible == -1 || maxSampleRatePossible == -1) throw new LibraryJavaSound.Exception("No possible " + "sample-rate found for Mixer '" + mixerInfo.getName() + "'", this);
      ...
      for (x = 1; x < lines.length + 1; x++) {
        try {
          ...
          if (x == 0) throw new LibraryJavaSound.Exception("No output " + "lines possible for Mixer '" + mixerInfo.getName() + "'", this);
          else if (LINE_COUNT_PRIORITY == HIGH) throw new LibraryJavaSound.Exception("Specified " + "maximum number of lines not possible for Mixer '" + mixerInfo.getName() + "'", this);
          break;
        }
      ...
      if (!useGainControl(GET, false)) {
        ...
        if (GAIN_CONTROL_PRIORITY == HIGH) throw new LibraryJavaSound.Exception("Gain control " + "not available for Mixer '" + mixerInfo.getName() + "'", this);
      } else {
        gainControlOK = true;
      }
      if (!usePanControl(GET, false)) {
        ...
        if (PAN_CONTROL_PRIORITY == HIGH) throw new LibraryJavaSound.Exception("Pan control " + "not available for Mixer '" + mixerInfo.getName() + "'", this);
      } else {
        panControlOK = true;
      }
      if (!useSampleRateControl(GET, false)) {
        ...
        if (SAMPLE_RATE_CONTROL_PRIORITY == HIGH) throw new LibraryJavaSound.Exception("Sample-rate " + "control not available for " + "Mixer '" + mixerInfo.getName() + "'", this);
      } else {
